...
splist=pd.read_csv(WD+"/1_hybpiper/""/namelist_"".txt",names=["tag"]) 

for index,row in splist.iterrows(): #iterate over TAGs
    tag=row[0] #take TAG
	#open coverage map corresponding to this TAG
    cov_file=pd.read_csv(WD+"/1.1_coverage_maps/""/per_TAG/"+tag+"_supercontigs_sorted.coverage",sep="\t",names=["gene","pos","cov"]) 
    for gene_i in cov_file["gene"].unique(): #iterate over genes in coverage map
        ...
        select["gene"]=[tag]*len(select["gene"]) #replace column "gene" by "tags" for selected TAGX-geneX combination #IGNORE WARNING MESSAGE
