...
View(selected)
plot(read.tree(text=as.character(selected$Topology[4]))) #4: one of the other alternatives (not the major one) # you can check with the other topologies, Podococcus is always placed as sister with a few Oranias # but always supported only by one gene tree!
dev.off()
