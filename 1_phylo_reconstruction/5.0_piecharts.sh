...
for gene_tree in $WD/3_gene_trees/"$dir_value"4_collapsed/*.raxml.support.coll; do nw_reroot $gene_tree $outgroup > ${gene_tree/.raxml.support.coll}_rooted.support.coll; done 
...
java -jar $WD/programs/phyparts/target/phyparts-0.0.1-SNAPSHOT-jar-with-dependencies.jar -a 1 -v -d $WD/3_gene_trees/"$dir_value"4_collapsed/all_genes_rooted.raxml.support.coll -m $WD/4_coalescent_trees/"$dir_value"coalescent_lpp.tree -o $WD/5_phypartstopiecharts/$dir_value
...
python $GWD/scripts/1_phylo_reconstruction/phypartstopiecharts.py $WD/4_coalescent_trees/"$dir_value"coalescent_lpp.tree $WD/5_phypartstopiecharts/$dir_value gene_num --svg_name $WD/5_phypartstopiecharts/$dir_value/phypartspiecharts.svg
